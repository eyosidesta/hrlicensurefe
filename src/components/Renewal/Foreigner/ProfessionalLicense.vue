...
export default {
  ...
  setup(props, { emit }) {
    ...
    const submit = () => {
      ...
      store.dispatch("renewal/setProfessionalLicense", letterFile);
    };
    const submitBack = () => {
    ...
    onMounted(() => {
      ...
      letterFileBack = store.getters["renewal/getProfessionalLicense"];
      if (
        letterFileBack &&
      ...
      if (acceptedFields != null && acceptedFields.includes("APLFCO")) {
      ...
      if (route.params.id) {
        ...
        for (let i = 0; i < draftData.documents.length; i++) {
          if (draftData.documents[i].documentTypeCode == "APLFCO") {
            showUpload.value = false;
