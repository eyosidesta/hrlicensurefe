<template>
  <div>
    <!-- <reviewer-nav-bar tab="newLicenseDeclinedPayment" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="New License Declined Payments" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <declined-payment-applications
          :declinedPaymentApplication="getNewLicenseDeclinedPayments"
...
export default {
  ...
  setup() {
    ...
    const fetchNewLicenseDeclinedPayment = () => {
      showLoading.value = true;
      const statusId = applicationStatus(store, 'DP');
      const adminStatus = [statusId, adminId];
      store
        .dispatch("reviewerNewLicense/getNewLicenseDeclinedPayment", adminStatus)
        .then((res) => {
    ...
    onMounted(() => {
      fetchNewLicenseDeclinedPayment();
    });
