  ...
  <div>
    ...
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="New License Assigned To You" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <assigned-applications :assignedApplication="getNewLicenseAssigned" app_type="New License" assigned_to_others="false"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    ...
    getNewLicenseAssigned() {
      return store.getters["reviewerNewLicense/getNewLicenseAssignedToYouSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchNewLicenseAssigned = () => {
      ...
      store.dispatch("reviewerNewLicense/getNewLicenseAssigned", adminStatus).then((res) => {
        ...
        if (store.getters["reviewerNewLicense/getNewLicenseAssignedToYou"].length === 0) {
