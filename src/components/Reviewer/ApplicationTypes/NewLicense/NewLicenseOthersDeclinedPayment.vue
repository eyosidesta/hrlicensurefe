<template>
  <div>
    <!-- <reviewer-nav-bar tab="othersNewLicenseDeclinedPayment" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Others New License Declined Payments" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <declined-payment-applications
          :declinedPaymentApplication="getNewLicenseOthersDeclinedPayments"
...
export default {
  computed: {
    moment: () => moment,
    getNewLicenseOthersDeclinedPayments() {
  ...
  setup() {
    ...
    let newLicenseDeclinedPayments = ref([]);
    ...
    const fetchOthersNewLicenseDeclinedPayment = () => {
      showLoading.value = true;
      const statusId = applicationStatus(store, 'DP');
      const adminStatus = [statusId, adminId];
      store
        .dispatch("reviewerNewLicense/getNewLicenseOthersDeclinedPayment", adminStatus)
        .then((res) => {
          showLoading.value = false;
          newLicenseDeclinedPayments.value =
            store.getters[
              "reviewerNewLicense/getNewLicenseOthersDeclinedPaymentSearched"
            ];
          ...
          if (
            newLicenseDeclinedPayments.value.length === 0
          ) {
    ...
    onMounted(() => {
      fetchOthersNewLicenseDeclinedPayment();
    });
