<template>
  <div>
    <!-- <reviewer-nav-bar tab="newLicenseOthersDeclineConfirmed" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="New License Others Decline Confirmed" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <confirmed-applications
          :confirmedApplication="getNewLicenseOthersDeclineConfirmed"
    ...
    <div class="bg-lightBlueB-200 h-full" v-if="allInfo.searchByInput">
      <div class="flex pl-12 pt-tiny">
        <Title
          :message="
            'Others Decline Confirmed Applicants on Date Range ' +
...
export default {
  computed: {
    moment: () => moment,
    getNewLicenseOthersDeclineConfirmed() {
      return store.getters[
        "reviewerNewLicense/getNewLicenseOthersDeclineConfirmedSearched"
      ];
    },
  },
  components: {
  ...
  setup() {
    ...
    let newLicenseOthersDeclineConfirmed = ref([]);
    ...
    const fetchNewLicenseOthersDeclineConfirmed = () => {
      ...
      store.dispatch(
          ...
          if (newLicenseOthersDeclineConfirmed.value.length === 0) {
            nothingToShow.value = true;
          }
        });
    };
    onMounted(() => {
      fetchNewLicenseOthersDeclineConfirmed();
    });
