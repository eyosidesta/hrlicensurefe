<template>
  <div>
    <!-- <reviewer-nav-bar tab="newLicenseOthersPendingPayment" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="New License Others Pending Payment" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <pending-payment-applications :pendingPaymentApplication="getNewLicenseOthersPendingPayment" app_type="New License" others_pending_payment="true"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    moment: () => moment,
    getNewLicenseOthersPendingPayment() {
      return store.getters["reviewerNewLicense/getNewLicenseOthersPendingPaymentSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchNewLicenseOthersPendingPayment = () => {
      showLoading.value = true;
      const statusId = applicationStatus(store, 'PP');
      const adminStatus = [statusId, adminId];
      store.dispatch("reviewerNewLicense/getNewLicenseOthersPendingPayment", adminStatus).then((res) => {
    ...
    onMounted(() => {
      fetchNewLicenseOthersPendingPayment();
    });
