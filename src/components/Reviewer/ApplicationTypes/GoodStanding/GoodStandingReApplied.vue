<template>
  <div>
    <!-- <reviewer-nav-bar tab="goodStandingReApplied" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        ...
        <re-applied-applications :reappliedApplication="getGoodStandingReApply" app_type="Good Standing"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    moment: () => moment,
    getGoodStandingReApply() {
      return store.getters["reviewerGoodStanding/getGoodStandingReApplySearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchGoodStandingReApplied = () => {
      ...
      store.dispatch("reviewerGoodStanding/getGoodStandingReApply", adminStatus).then((res) => {
        ...
        if (goodStandingReapplied.value.length === 0) {
