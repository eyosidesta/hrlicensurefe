<template>
  <div>
    <!-- <reviewer-nav-bar tab="goodStandingLicensed" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Good Standing Licensed" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <licensed-applications :licensedApplication="getGoodStandingLicensed" app_type="Good Standing" others_licensed="false"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    moment: () => moment,
    getGoodStandingLicensed() {
      return store.getters["reviewerGoodStanding/getGoodStandingLicensedSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchGoodStandingLicensed = () => {
      ...
      store.dispatch("reviewerGoodStanding/getGoodStandingLicensed", adminStatus).then((res) => {
        showLoading.value = false;
        goodStandingLicensed.value =
          store.getters["reviewerGoodStanding/getGoodStandingLicensedSearched"];
        ...
        if (goodStandingLicensed.value.length === 0) {
          nothingToShow.value = true;
        }
      });
    };
    onMounted(() => {
      fetchGoodStandingLicensed();
    });
