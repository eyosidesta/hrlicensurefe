<template>
  <div>
    <!-- <reviewer-nav-bar tab="goodStandingOthersLicensed" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Good Standing Others Licensed" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <licensed-applications :licensedApplication="getGoodStandingOthersLicensed" app_type="Good Standing" others_licensed="true"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    moment: () => moment,
    getGoodStandingOthersLicensed() {
      return store.getters["reviewerGoodStanding/getGoodStandingOthersLicensedSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    let goodStandingOthersLicensed = ref([]);
    ...
    const fetchGoodStandingOthersLicensed = () => {
      ...
      store.dispatch("reviewerGoodStanding/getGoodStandingOtherLicensed", adminStatus).then((res) => {
        ...
        if (goodStandingOthersLicensed.value.length === 0) {
          nothingToShow.value = true;
        }
      });
    };
    onMounted(() => {
      fetchGoodStandingOthersLicensed();
    });
