<template>
  <div>
    <!-- <reviewer-nav-bar tab="goodStandingAssigned" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Good Standing Assigned To You" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <assigned-applications :assignedApplication="getGoodStandingAssigned" app_type="Good Standing" assigned_to_others="false"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    ...
    getGoodStandingAssigned() {
      return store.getters["reviewerGoodStanding/getGoodStandingAssignedToYouSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchGoodStandingseAssigned = () => {
      ...
      store.dispatch("reviewerGoodStanding/getGoodStandingAssigned", adminStatus).then((res) => {
        showLoading.value = false;
        goodStandingAssigned.value =
          store.getters["reviewerGoodStanding/getGoodStandingAssignedToYouSearched"];
        ...
        if (store.getters["reviewerGoodStanding/getGoodStandingAssignedToYou"].length === 0) {
