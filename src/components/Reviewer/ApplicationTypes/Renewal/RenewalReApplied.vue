<template>
  <div>
    <!-- <reviewer-nav-bar tab="renewalReApplied" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Renewal Reapplied" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <re-applied-applications :reappliedApplication="getRenewalReApply" app_type="Renewal"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    moment: () => moment,
    getRenewalReApply() {
      return store.getters["reviewerRenewal/getRenewalReApplySearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchRenewalReApplied = () => {
      showLoading.value = true;
      const statusId = applicationStatus(store, 'UPD');
      const adminStatus = [statusId, adminId];
      store.dispatch("reviewerRenewal/getRenewalReApply", adminStatus).then((res) => {
        showLoading.value = false;
        renewalReapplied.value =
        ...
        if (renewalReapplied.value.length === 0) {
