<template>
  <div>
    <!-- <reviewer-nav-bar tab="renewalPendingPayment" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Renewal Pending Payment" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <pending-payment-applications :pendingPaymentApplication="getRenewalPendingPayment" app_type="Renewal" others_pending_payment="false"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    ...
    getRenewalPendingPayment() {
      return store.getters["reviewerRenewal/getRenewalPendingPaymentSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchRenewalPendingPayment = () => {
      showLoading.value = true;
      const statusId = applicationStatus(store, 'PP');
      const adminStatus = [statusId, adminId];
      store.dispatch("reviewerRenewal/getRenewalPendingPayment", adminStatus).then((res) => {
        showLoading.value = false;
        renewalPendingPayment.value =
        ...
        if (renewalPendingPayment.value.length === 0) {
          nothingToShow.value = true;
        }
      });
    };
    onMounted(() => {
      fetchRenewalPendingPayment();
    });
