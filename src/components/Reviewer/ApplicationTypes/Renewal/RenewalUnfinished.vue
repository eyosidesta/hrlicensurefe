<template>
  <div>
    <!-- <reviewer-nav-bar tab="renewalUnfinished" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Renewal Unfinished" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <unfinished-applications :unfinishedApplication="getRenewalUnfinished" app_type="Renewal" others_unfinished="false"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    moment: () => moment,
    getRenewalUnfinished() {
      return store.getters["reviewerRenewal/getRenewalUnfinishedSearched"];
    },
  },
  components: {
    ErrorFlashMessage,
  ...
  setup() {
    ...
    const fetchRenewalUnfinished = () => {
      ...
      store.dispatch("reviewerRenewal/getRenewalUnfinished", adminStatus).then(() => {
        ...
        if (store.getters["reviewerRenewal/getRenewalUnfinished"].length === 0) {
