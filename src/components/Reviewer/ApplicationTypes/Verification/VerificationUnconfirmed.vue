<template>
  <div>
    <!-- <reviewer-nav-bar tab="verificationUnconfirmed" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        ...
        <unconfirmed-applications
          :unconfirmedApplication="getVerificationUnconfirmed"
...
export default {
  ...
  setup() {
    ...
    const fetchVerificationUnconfirmed = () => {
      ...
      store
        .dispatch("reviewerVerification/getVerificationApproved", adminStatus)
        .then((res) => {
          ...
          if (
            store.getters["reviewerVerification/getVerificationApproved"]
              .length === 0
          ) {
