<template>
  <div>
    <!-- <reviewer-nav-bar tab="verificationAssigned" /> -->
    <div class="bg-lightBlueB-200 h-full" v-if="!allInfo.searchByInput">
      ...
      <div class="flex pl-12 pt-tiny">
        <Title message="Verification Assigned To You" />
      </div>
      <div class="flex flex-wrap pb-medium rounded h-full" v-if="!showLoading">
        <nothing-to-show :nothingToShow="nothingToShow" />
        <assigned-applications :assignedApplication="getVerificationAssigned" app_type="Verification" assigned_to_others="false"/>
      </div>
    </div>
    <div
      v-if="showLoading"
...
export default {
  computed: {
    ...
    getVerificationAssigned() {
      return store.getters["reviewerVerification/getVerificationAssignedToYouSearched"];
    },
  },
  components: {
    ReviewerNavBar,
  ...
  setup() {
    ...
    const fetchVerificationAssigned = () => {
      ...
      store.dispatch("reviewerVerification/getVerificationAssigned", adminStatus).then((res) => {
        showLoading.value = false;
        verificationAssigned.value =
          store.getters["reviewerVerification/getVerificationAssignedToYouSearched"];
        ...
        if (store.getters["reviewerVerification/getVerificationAssignedToYou"].length === 0) {
