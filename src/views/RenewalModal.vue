<template>
  ...
  >
    <Title message="You should apply for a New License First." />
    <button
      class="absolute top-0 right-0 mr-2 mt-2"
      @click="$emit('showRenewalModal', false)"
      variant="rounded"
    >
      <svg
    ...
    <!-- <div>
      <div class="ml-8">
        <h2>You should apply for a New License First.</h2>
        <ul>
    ...
    <router-link to="/menu">
      <h2 class="mt-4">Continue to New License</h2>
    </router-link>
  </div>
</template>
<script>
